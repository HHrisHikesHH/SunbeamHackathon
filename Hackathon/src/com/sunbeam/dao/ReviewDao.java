...
public class ReviewDao implements AutoCloseable {
	...
	public ReviewDao() throws SQLException {
		stmtfindAllReviewIdsbyUserId = connection.prepareStatement("SELECT REVIEW_ID FROM REVIEWS WHERE USER_ID = ?");
		stmteditReview = connection.prepareStatement("UPDATE REVIEWS SET REVIEW = ? , RATING = ? WHERE REVIEW_ID = ?");
		stmtfindByReviewId = connection.prepareStatement("SELECT * FROM REVIEWS WHERE REVIEW_ID = ?");
		stmtfindAllIdMovies = connection.prepareStatement("SELECT MOVIE_ID FROM MOVIES");
		stmtfindAllId = connection.prepareStatement("SELECT REVIEW_ID FROM REVIEWS");
		stmtdeletebyId1 = connection.prepareStatement("DELETE FROM SHARES WHERE REVIEW_ID = ?");
		stmtdeletebyId2 = connection.prepareStatement("DELETE FROM REVIEWS WHERE REVIEW_ID = ?");
		stmtfindAllMyReviews = connection
				.prepareStatement(" SELECT * FROM REVIEWS WHERE REVIEW_ID = ANY(SELECT REVIEW_ID FROM SHARES\n"
						+ "		 WHERE USER_ID = ?)");
	...
	public List<Review> getAllReviews() throws SQLException {
		...
		try (ResultSet rs = stmtfindAll.executeQuery()) {
			while (rs.next()) {
				reviews.add(new Review(rs.getInt("review_id"), rs.getInt("movie_id"), rs.getString("review"),
...
//		System.out.println(reviews);
	...
	public boolean checkIfAvailable(int reviewId) throws SQLException {
		...
		try (ResultSet rs = stmtfindAllId.executeQuery()) {
			while (rs.next()) {
				reviews.add(rs.getInt("review_id"));
			}
		} // rs.close()
	...
	public Review getReviewById(int reviewId) throws SQLException {
		...
		try (ResultSet rs = stmtfindByReviewId.executeQuery()) {
			if (rs.next()) {
				return new Review(rs.getInt("review_id"), rs.getInt("movie_id"), rs.getString("review"),
						rs.getInt("rating"), rs.getInt("user_id"), new Date(rs.getDate("modified").getTime()));
			}
		}
		return null;
