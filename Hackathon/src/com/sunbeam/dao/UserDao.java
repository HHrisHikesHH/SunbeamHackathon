...
public class UserDao implements AutoCloseable {
	...
	public UserDao() throws SQLException {
		...
		stmtfindById = connection.prepareStatement("SELECT USER_ID FROM USERS WHERE USER_ID = ?");
		stmtfindPassById = connection.prepareStatement("SELECT PASSWORD FROM USERS WHERE USER_ID = ?");
		...
		stmtupdateById = connection.prepareStatement("UPDATE USERS SET first_name = ? , "
				+ "last_name = ?, email = ? ,mobile = ?, birth = ? WHERE USER_ID = ?");
		stmtupdatePasswordById = connection.prepareStatement("UPDATE USERS SET password = ? WHERE USER_ID = ?");
	}
